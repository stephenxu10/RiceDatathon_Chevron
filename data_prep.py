...
def prepare_train_data(overall_data, states):
    input_data = torch.ones(5 * len(states), 19, device=device)
    ...
    input_data = torch.from_numpy(states_data.to_numpy()).reshape(len(states), 5, 19).float()
    return input_data, actual


def prepare_test_data(overall_data):
    input_data = torch.ones(50, 19, device=device)
    ...
    input_data = torch.from_numpy(overall_data.to_numpy()).reshape(50, 1, 19)
    return input_data, actual
